...
final class ClaudeStatusProvider: AgentStatusProvider {
    ...
    BACKGROUND JOBS: a long-running job whose output is VISIBLY PROGRESSING — a training run with a climbing step count, a sweep with a rising %, an rsync with files ticking up, fresh log lines — counts as ACTIVE even when the agent itself is idle at the prompt; report the job's state and progress ("training step 393/400, val healthy"), label working. BUT: a count of shells or processes merely existing ("17 shells still running") is NOT progress and NOT "working" on its own — only count a job as active if its output is actually advancing. And a pending question to the user, OR the agent being halted on an error (see "stuck"), ALWAYS takes priority over a running job — report that the agent needs you, not the job's %. A job's pace dropping sharply is a corroborating sign the agent stalled. MONITORING LOOP: if the agent LAUNCHED long-running work and is now MINDING it on a schedule — it stated a plan to keep checking and report ("I'll monitor every 30 min", "next fire ~HH:MM", "I'll report when it completes", "the loop continues"), or a scheduled/cron task keeps pulling it back — then the task is IN PROGRESS: label working, even if THIS snapshot is just a between-checks status report with no live motion on screen. This is the common case of a session sitting at the ❯ box while a real job runs under the agent's watch; it is working, not idle. (Still idle only when nothing is actually running and there is no such loop or pending completion.)
