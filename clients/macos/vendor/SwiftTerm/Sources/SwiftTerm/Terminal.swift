...
//                break
    ...
    private func snapshotBuffer (_ source: Buffer) -> Buffer
    {
        ...
        // the cost (O(scrollback) per frame × every terminal). Deep-copy just the
        // viewport (which the live buffer overwrites while we're frozen) and share the
        // scrollback lines by reference — they're never mutated until trimmed, and a
        // trimmed line stays alive via this reference.
        let viewportStart = max(0, source.yBase)
        for idx in 0..<source.lines.count {
            if idx >= viewportStart {
                copy.lines.push(BufferLine(from: source.lines[idx]))
            } else {
                copy.lines.push(source.lines[idx])
            }
        }
        return copy
    }
